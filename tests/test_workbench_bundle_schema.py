...
VALIDATION_SCRIPT_PATH = PROJECT_ROOT / "tools" / "validate_workbench_bundle_schema.py"
FORCE_JSONSCHEMA_MISSING_ENV = "WELL_HARNESS_FORCE_JSONSCHEMA_MISSING"


def workbench_bundle_schema_script_env(env_overrides=None):
    env = dict(os.environ)
...
class WorkbenchBundleSchemaValidationScriptTests(unittest.TestCase):
    def test_workbench_bundle_schema_standalone_script_smoke(self):
        result = subprocess.run(
            [sys.executable, str(VALIDATION_SCRIPT_PATH)],
            cwd=PROJECT_ROOT,
            env=workbench_bundle_schema_script_env(),
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        self.assertIn("PASS: validated 3 workbench bundle payloads", result.stdout)
        self.assertIn("docs/json_schema/workbench_bundle_v1.schema.json", result.stdout)
    ...
    def test_workbench_bundle_schema_standalone_script_json_pass_output(self):
        try:
        ...
        self.assertEqual("docs/json_schema/workbench_bundle_v1.schema.json", payload["schema_path"])
    ...
    def test_workbench_bundle_schema_standalone_script_json_skip_output(self):
        result = subprocess.run(
            [sys.executable, str(VALIDATION_SCRIPT_PATH), "--format", "json"],
            cwd=PROJECT_ROOT,
            env=workbench_bundle_schema_script_env({FORCE_JSONSCHEMA_MISSING_ENV: "1"}),
            capture_output=True,
            text=True,
            check=False,
        )
