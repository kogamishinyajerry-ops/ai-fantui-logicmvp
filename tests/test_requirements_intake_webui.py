...
def test_logic_builder_static_page_exposes_streamed_authoring_controls():
    ...
    assert 'data-current-segment-jump="all"' in html
    ...
    assert 'id="logic-global-review-matrix"' in html
    assert 'data-global-review-matrix="circuit-evidence"' in html
    assert 'data-review-item="source"' in html
...
def test_requirements_intake_static_subproject_hooks_exist():
    ...
    assert "return NODE_KIND_LABELS[normalized] || fallback;" in logic_script
    ...
    assert "return NODE_KIND_LABELS[normalized] || normalized;" not in logic_script
    ...
    assert 'function circuitRoleLabel(value, fallback = "角色待确认")' in logic_script
    assert "return CIRCUIT_ROLE_LABELS[value] || fallback;" in logic_script
    ...
    assert "return CIRCUIT_ROLE_LABELS[value] || value || \"\";" not in logic_script
    ...
    assert 'const evidence = linkTarget.evidence || meta || "候选证据"' in sandbox_script
    assert 'decision: "证据回链"' in sandbox_script
    assert 'linkTarget.decision || "LINK"' not in sandbox_script
    assert 'linkTarget.evidence || meta || "candidate"' not in sandbox_script
...
def test_logic_builder_exposes_cockpit_annotation_stream_surface():
    ...
    assert "requirementsPatchHashPrefix" in script
    assert "补丁证据已记录" in script
