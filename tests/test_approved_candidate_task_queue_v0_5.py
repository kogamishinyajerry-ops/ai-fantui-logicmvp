...
SCHEMA_PATH = (
    ...
    / "docs"
    / "json_schema"
    / "approved_candidate_task_queue_summary_v0_5.schema.json"
)
RUN_SCRIPT_PATH = PROJECT_ROOT / "scripts" / "run_approved_candidate_task_queue_v0_5.py"
CHECKER_SCRIPT_PATH = (
    PROJECT_ROOT / "scripts" / "verify_approved_candidate_task_queue_v0_5_artifact.py"
)
...
def test_v0_5_queue_runner_and_checker_append_unknown_requirement_repair_record(
    tmp_path: Path,
) -> None:
    ...
    assert payload["queue_id"] == "approved-candidate-task-queue-v0.5"
    assert payload["queue_order"][:6] == V0_4_QUEUE_ORDER
    assert payload["queue_order"][6] == M17_QUEUE_ITEM_ID
    ...
    new_item = payload["items"][6]
    assert new_item["queue_item_id"] == M17_QUEUE_ITEM_ID
    ...
    assert new_item["approved_task_shell"] == {
        ...
        "approval_status": "approved",
        "selected_task_id": "TASK-CE-EV-TEST-COVERS-UNKNOWN-REQUIREMENT",
    }
    ...
    assert chain["finding"]["code"] == "EV_TEST_COVERS_UNKNOWN_REQUIREMENT"
    assert chain["approval"]["approval_id"] == (
        "APPROVAL-EVIDENCE-UNKNOWN-REQUIREMENT-REPAIR-SLICE-001"
    )
...
def test_v0_5_queue_checker_rejects_missing_unknown_requirement_item(
    tmp_path: Path,
) -> None:
    result = subprocess.run(
        [
    ...
    summary["items"] = summary["items"][:6]
    summary["queue_order"] = summary["queue_order"][:6]
...
def test_v0_5_queue_make_targets_are_wired() -> None:
    makefile = MAKEFILE_PATH.read_text(encoding="utf-8")

    assert "approved-candidate-task-queue-v0-5" in makefile
    assert "verify-approved-candidate-task-queue-v0-5-artifact" in makefile
    assert "scripts/run_approved_candidate_task_queue_v0_5.py" in makefile
    assert "scripts/verify_approved_candidate_task_queue_v0_5_artifact.py" in makefile
