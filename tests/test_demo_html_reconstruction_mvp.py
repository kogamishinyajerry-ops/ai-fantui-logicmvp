...
def test_demo_html_reconstruction_browser_acceptance_script_captures_interaction_evidence(
    ...
    assert "目标 反推锁释放" in payload["scenario_comparator_review"]["maxText"]
    assert "THR ON" not in payload["scenario_comparator_review"]["maxText"]
    assert "抑制阻塞" in payload["scenario_comparator_review"]["inhibitText"]
    assert "目标 反推锁阻塞" in payload["scenario_comparator_review"]["inhibitText"]
    assert "THR BLOCKED" not in payload["scenario_comparator_review"]["inhibitText"]
    ...
    assert "展开可用" in payload["scenario_comparator_max_review"]["readbackText"]
    assert "反推锁释放" in payload["scenario_comparator_max_review"]["readbackText"]
    assert "解锁电源通电" in payload["scenario_comparator_max_review"]["readbackText"]
    assert "作动器供电" in payload["scenario_comparator_max_review"]["readbackText"]
    assert "THR ON" not in payload["scenario_comparator_max_review"]["readbackText"]
    ...
    assert "安全阻塞" in payload["scenario_comparator_inhibit_review"]["readbackText"]
    assert "反推锁阻塞" in payload["scenario_comparator_inhibit_review"]["readbackText"]
    assert "解锁电源关闭" in payload["scenario_comparator_inhibit_review"]["readbackText"]
    assert "THR BLOCKED" not in payload["scenario_comparator_inhibit_review"]["readbackText"]
