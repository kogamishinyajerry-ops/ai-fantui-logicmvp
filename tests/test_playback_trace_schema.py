...
VALIDATION_SCRIPT_PATH = PROJECT_ROOT / "tools" / "validate_playback_trace_schema.py"
FORCE_JSONSCHEMA_MISSING_ENV = "WELL_HARNESS_FORCE_JSONSCHEMA_MISSING"


def playback_trace_schema_script_env(env_overrides=None):
    env = dict(os.environ)
...
class PlaybackTraceSchemaValidationScriptTests(unittest.TestCase):
    def test_playback_trace_schema_standalone_script_smoke(self):
        result = subprocess.run(
            [sys.executable, str(VALIDATION_SCRIPT_PATH)],
            cwd=PROJECT_ROOT,
            env=playback_trace_schema_script_env(),
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        self.assertIn("PASS: validated 2 playback trace payloads", result.stdout)
        self.assertIn("docs/json_schema/playback_trace_v1.schema.json", result.stdout)

    def test_playback_trace_schema_standalone_script_json_pass_output(self):
        try:
    ...
    def test_playback_trace_schema_standalone_script_json_skip_output(self):
        result = subprocess.run(
            [sys.executable, str(VALIDATION_SCRIPT_PATH), "--format", "json"],
            cwd=PROJECT_ROOT,
            env=playback_trace_schema_script_env({FORCE_JSONSCHEMA_MISSING_ENV: "1"}),
            capture_output=True,
            text=True,
            check=False,
        )
