...
SCHEMA_PATH = PROJECT_ROOT / "docs" / "json_schema" / "fault_diagnosis_v1.schema.json"
...
OPTIONAL_JSONSCHEMA_SKIP_MESSAGE = (
    ...
    "Install it to validate fault-diagnosis payloads."
)
CASES = (
    {
        "name": "fixture_packet",
...
def run_diagnosis_json(case: dict) -> tuple[int, dict]:
    buffer = io.StringIO()
    ...
    try:
        ...
        raise ValueError(f"unable to parse diagnosis JSON for case {case['name']!r}: {exc}") from exc
...
def validate_fault_diagnosis_payloads() -> tuple[int, dict, list[str]]:
    report = make_report()
    ...
    for case in CASES:
        try:
            exit_code, payload = run_diagnosis_json(case)
        ...
        if exit_code != 0:
            ...
            report["results"].append(
                make_result(case, "fail", error_count=1, errors=[f"diagnose-fault CLI exited with status {exit_code}"])
            )
            report["status"] = "fail"
            return 1, report, [f"FAIL {case['name']}: diagnose-fault CLI exited with status {exit_code}"]
        ...
        if errors:
            ...
            report["status"] = "fail"
            text_lines.append(f"FAIL {case['name']}: fault diagnosis schema validation errors")
        ...
        if payload.get("fault_mode_id") != case["fault_mode_id"]:
            report["failure_kind"] = "fault_mode_id_mismatch"
            report["results"].append(
                make_result(
                    case,
                    "fail",
                    error_count=1,
                    errors=[f"expected fault_mode_id {case['fault_mode_id']} but got {payload.get('fault_mode_id')}"],
                )
            )
            report["status"] = "fail"
            return 1, report, [
                f"FAIL {case['name']}: expected fault_mode_id {case['fault_mode_id']} but got {payload.get('fault_mode_id')}"
            ]

        report["results"].append(make_result(case, "pass", error_count=0, errors=[]))
        text_lines.append(
            f"OK {case['name']}: validated fault diagnosis packet={relative_path(case['packet_path'])}"
        )

    text_lines.append(
        f"PASS: validated {len(CASES)} fault diagnosis payloads against {relative_path(SCHEMA_PATH)}"
    )
...
def parse_output_format(argv: list[str]) -> str:
    ...
    if len(argv) == 2 and argv[0] == "--format" and argv[1] in OUTPUT_FORMATS:
        return argv[1]
    raise ValueError("usage: validate_fault_diagnosis_schema.py [--format text|json]")
...
def main(argv: list[str] | None = None) -> int:
    ...
    exit_code, report, text_lines = validate_fault_diagnosis_payloads()
