...
SCHEMA_PATH = PROJECT_ROOT / "docs" / "json_schema" / "playback_trace_v1.schema.json"
...
OPTIONAL_JSONSCHEMA_SKIP_MESSAGE = (
    ...
    "Install it to validate playback-trace payloads."
)
CASES = (
    {
        "name": "fixture_packet",
...
def run_playback_json(case: dict) -> tuple[int, dict]:
    buffer = io.StringIO()
    ...
    try:
        ...
        raise ValueError(f"unable to parse playback JSON for case {case['name']!r}: {exc}") from exc
...
def validate_playback_payloads() -> tuple[int, dict, list[str]]:
    report = make_report()
    ...
    for case in CASES:
        try:
            exit_code, payload = run_playback_json(case)
        except ValueError as exc:
            report["failure_kind"] = "payload_parse"
            report["results"].append(
                make_result(case, "fail", error_count=1, errors=[str(exc)])
            )
            report["status"] = "fail"
        ...
        if exit_code != 0:
            ...
            report["results"].append(
                make_result(
                    ...
                    "fail",
                    error_count=1,
                    errors=[f"playback CLI exited with status {exit_code}"],
                )
            )
            report["status"] = "fail"
            return 1, report, [f"FAIL {case['name']}: playback CLI exited with status {exit_code}"]
        ...
        if errors:
            ...
            report["results"].append(
                make_result(case, "fail", error_count=len(errors), errors=formatted_errors)
            )
            report["status"] = "fail"
            text_lines.append(f"FAIL {case['name']}: playback trace schema validation errors")
        ...
        if payload.get("scenario_id") != case["scenario_id"]:
            report["failure_kind"] = "scenario_id_mismatch"
            report["results"].append(
                make_result(
                    case,
                    "fail",
                    error_count=1,
                    errors=[f"expected scenario_id {case['scenario_id']} but got {payload.get('scenario_id')}"],
                )
            )
            report["status"] = "fail"
            return 1, report, [
                f"FAIL {case['name']}: expected scenario_id {case['scenario_id']} but got {payload.get('scenario_id')}"
            ]

        report["results"].append(
            make_result(case, "pass", error_count=0, errors=[])
        )
        text_lines.append(
            f"OK {case['name']}: validated playback trace packet={relative_path(case['packet_path'])}"
        )

    text_lines.append(
        f"PASS: validated {len(CASES)} playback trace payloads against {relative_path(SCHEMA_PATH)}"
    )
...
def parse_output_format(argv: list[str]) -> str:
    ...
    if len(argv) == 2 and argv[0] == "--format" and argv[1] in OUTPUT_FORMATS:
        return argv[1]
    raise ValueError("usage: validate_playback_trace_schema.py [--format text|json]")
...
def main(argv: list[str] | None = None) -> int:
    ...
    exit_code, report, text_lines = validate_playback_payloads()
