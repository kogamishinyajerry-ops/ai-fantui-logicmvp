...
SCHEMA_PATH = PROJECT_ROOT / "docs" / "json_schema" / "validation_schema_runner_report_v1.schema.json"
ASSET_PATH = PROJECT_ROOT / "tests" / "fixtures" / "validation_schema_runner_report_asset_v1.json"
VALIDATION_SCRIPT_PATH = PROJECT_ROOT / "tools" / "validate_validation_report_schema.py"
...
def run_validation_schema_runner_scenario(asset: dict, scenario_name: str) -> tuple[subprocess.CompletedProcess[str], dict]:
    ...
    try:
        ...
        raise ValueError(f"unable to parse validation schema runner report JSON for {scenario_name}: {exc}") from exc
...
def make_result(scenario_name: str, payload_status: str | None, validation_status: str, **extra_fields) -> dict:
    result = {
...
def validate_reports() -> tuple[int, dict, list[str]]:
    ...
    for scenario_name in ("pass", "skip", "fail"):
        scenario = asset[scenario_name]
        try:
            result, payload = run_validation_schema_runner_scenario(asset, scenario_name)
        except ValueError as exc:
            report["failure_kind"] = "payload_parse"
            report["results"].append(
                make_result(scenario_name, None, "fail", reason=str(exc))
            )
            report["status"] = "fail"
            return 1, report, [f"FAIL {scenario_name}: {exc}"]
        ...
        if errors:
            ...
            report["status"] = "fail"
            text_lines.append(f"FAIL {scenario_name}: validation schema runner report schema errors")
        ...
        if payload.get("status") != scenario["expected_status"]:
            ...
            report["results"].append(
                make_result(
                    ...
                    payload.get("status"),
                    "fail",
                    expected_payload_status=scenario["expected_status"],
        ...
        if scenario["expected_status"] == "fail":
            if result.returncode == 0:
                ...
                report["status"] = "fail"
                return 1, report, [f"FAIL {scenario_name}: expected report command to fail"]
            ...
            report["status"] = "fail"
            return 1, report, [f"FAIL {scenario_name}: report command exited with status {result.returncode}"]
        ...
        text_lines.append(
            f"OK {scenario_name}: validated validation schema runner report status={payload['status']}"
        )

    text_lines.append(
        f"PASS: validated 3 validation schema runner report payloads against "
...
def parse_output_format(argv: list[str]) -> str:
    ...
    if len(argv) == 2 and argv[0] == "--format" and argv[1] in OUTPUT_FORMATS:
        return argv[1]
    raise ValueError("usage: validate_validation_schema_runner_report_schema.py [--format text|json]")
