...
status: Live Linear JER-256 JER-171 mypy baseline tranche 14 implementing
last_updated: "2026-05-05T22:21:50.000+08:00"
...
progress:
  ...
  total_plans: 2
  completed_plans: 1
  post_v5_closeout: "PR #216 through PR #220 merged repo-local JER-234 through JER-238 to main, PR #221 merged validation-suite timeout maintenance, and post-v5 main passed the bounded GSD validation suite 25/25. Live Linear JER-229 is the Linear/Symphony bootstrap issue, live Linear JER-230 is the first post-v5 v6 queue seed, live Linear JER-231 is the review archive library/recent restore surface slice, live Linear JER-232 is the issue factory/collision guard slice, live Linear JER-233 is the first JER-171 mypy baseline reduction tranche, live Linear JER-234 is the e2e readiness audit, live Linear JER-235 is the captured-template import gate fix, live Linear JER-236 is the large sandbox graph trace stability proof, live Linear JER-237 is the post-JER-236 queue refresh, live Linear JER-238 is the restore diff drilldown product slice, live Linear JER-239 is the large-graph stress pack proof slice, live Linear JER-240 is the second JER-171 mypy baseline reduction tranche, live Linear JER-241 is the post-JER-240 production-readiness queue refresh, live Linear JER-242 is the release-candidate workbench smoke pack, live Linear JER-243 is the post-JER-240 full opt-in e2e refresh, live Linear JER-244 is the local production runbook and release manifest slice, live Linear JER-245 is the third JER-171 mypy baseline reduction tranche, live Linear JER-246 is the fourth JER-171 mypy baseline reduction tranche, live Linear JER-247 is the fifth JER-171 mypy baseline reduction tranche, live Linear JER-248 is the sixth JER-171 mypy baseline reduction tranche, live Linear JER-249 is the seventh JER-171 mypy baseline reduction tranche, live Linear JER-250 is the eighth JER-171 mypy baseline reduction tranche, live Linear JER-251 is the ninth JER-171 mypy baseline reduction tranche, live Linear JER-252 is the tenth JER-171 mypy baseline reduction tranche, live Linear JER-253 is the eleventh JER-171 mypy baseline reduction tranche, live Linear JER-254 is the twelfth JER-171 mypy baseline reduction tranche, live Linear JER-255 is the thirteenth JER-171 mypy baseline reduction tranche, and live Linear JER-256 is the fourteenth JER-171 mypy baseline reduction tranche; live ids must be distinguished from repo-local historical JER labels."
...
closed the canonical graph document v2 boundary, JER-232 closed direct
port-drag wiring with route diagnostics after PR #214, JER-233 closed the
scenario test case library v1 slice after PR #215, and JER-234 through JER-238
are merged on `main` through PR #216 through PR #220. PR #221 also merged the
validation-suite timeout budget fix, so the default
`tools/run_gsd_validation_suite.py` command passed 25/25 on post-v5 `main`.
The active mainline is the
...
**2026-05-03 Linear/Symphony bootstrap**: live Linear issue `JER-229`
(`AI FANTUI LogicMVP · v6 next-queue bootstrap`) was created and received proof
comment `comment-09cd9a49`. This live Linear id collides with the repo-local
historical `JER-229` v5 launch slice, so future work must distinguish `live
Linear JER-229` from `repo-local JER-229`. See
`docs/coordination/linear-symphony-bootstrap.md`.

**2026-05-03 live v6 queue seed**: live Linear issue `JER-230`
(`[project] [L9] [none] [DAL-TBD] Post-v5 v6 live queue seed`) was created in
the `AI FANTUI LogicMVP · Codex Daily Lane` project and closed the post-v5
queue setup slice. This live Linear id collides with
the repo-local historical `JER-230` empty-canvas graph authoring slice, so
future work must distinguish `live Linear JER-230` from `repo-local JER-230`.
See `docs/coordination/post-v5-v6-live-queue.md`.

**2026-05-03 live v6 product slice**: live Linear issue `JER-231`
(`[project] [L4] [none] [DAL-TBD] Review archive library and recent restore
surface`) was created in the `AI FANTUI LogicMVP · Codex Daily Lane` project
as the first post-v5 product implementation slice and closed through PR #224.
This live Linear id
collides with the repo-local historical `JER-231` canonical graph document v2
slice, so future work must distinguish `live Linear JER-231` from `repo-local
JER-231`.

**2026-05-03 live v6 infrastructure slice**: live Linear issue `JER-232`
(`[project] [L9] [none] [DAL-TBD] Live Linear issue factory and collision
guard`) was created in the `AI FANTUI LogicMVP · Codex Daily Lane` project.
This live Linear id collides with the repo-local historical `JER-232` port drag
...
**2026-05-03 live v6 gate audit and fix**: live Linear issue `JER-234`
(`[project] [L9] [none] [DAL-TBD] Official e2e 49/49 readiness audit`) closed
through PR #227 after recording the real opt-in e2e surface as 91 selected
tests and finding the captured-template import blocker. live Linear issue
`JER-235` (`[project] [L4] [none] [DAL-TBD] Enable captured template insertion
after draft import`) closed through PR #228, restoring imported captured
subsystem template insertion and bringing the full opt-in e2e suite to 91
passed / 3439 deselected. These live ids collide with repo-local historical
`JER-234` and `JER-235`; future reports must use `live Linear` when needed.

**2026-05-05 live v6 scale proof and queue refresh**: live Linear issue
`JER-236` (`[project] [L6] [none] [DAL-TBD] Large sandbox graph trace
stability probe`) closed through PR #229 after adding large-graph e2e proof:
16-node trace kernels are deterministic, key archive checksums are stable, and
12-node invalid graphs keep structured unsupported-op, duplicate-edge, and
dangling-edge findings. The full opt-in e2e suite passed at 93 passed / 3439
deselected. live Linear issue `JER-237` closed through PR #230 with the
post-JER-236 queue refresh and next dispatch map. live Linear issue `JER-238`
closed through PR #231 with restore diff drilldown: mutated local review
archives report section, checksum key/path, expected/actual checksum, affected
evidence path, and `truth_effect: none`. live Linear issue `JER-239` closed
through PR #232 with reusable large-graph pass, fail, invalid graph, and
stale-report fixtures. live Linear issue `JER-240` closed through PR #233 with
the demo fault-injection test family focused strict-mypy clean and with the
official JER-171 wrapper reduced from 4913 errors in 347 files to 4617 errors
in 326 files while still blocked. live Linear issue `JER-241` is the current
production-readiness queue refresh and closed through PR #234 after recording
verified evidence honestly, naming remaining production blockers, and defining
the next Linear-ready contracts. live Linear issue `JER-242` is the current
product-readiness slice and closed through PR #235 with a local
release-candidate smoke gate that starts the demo server and probes the first
operator flows without external services or production-ready claims. live
Linear issue `JER-243` closed through PR #236 after current `main` reran the
full opt-in e2e command at 93 passed / 3445 deselected in 149.97s, with no
observed e2e blocker. live Linear issue `JER-244` is the current
release-operations slice for a local production-readiness runbook and
machine-readable release evidence manifest and closed through PR #237. live
Linear issue `JER-245` is the current JER-171 mypy baseline reduction tranche:
the scoped release-smoke tool import boundary is focused strict-mypy clean, and
the official wrapper moved from 4619 errors in 327 files to 4617 errors in 326
files while still blocked. live Linear issue `JER-246` is the current
validation-helper mypy tranche: the touched validation tools are focused
strict-mypy clean, and the official wrapper moved from 4617 errors in 326 files
to 4614 errors in 324 files while still blocked. live Linear issue `JER-247`
closed through PR #240 as the coordination/validation tool mypy tranche: the
touched tools are focused strict-mypy clean, and the official wrapper moved
from 4614 errors in 324 files to 4611 errors in 321 files while still blocked.
live Linear issue `JER-248` closed through PR #241 as the source
return-boundary mypy tranche: the touched source files are focused strict-mypy
clean, and the official wrapper moved from 4611 errors in 321 files to 4609
errors in 319 files while still blocked. live Linear issue `JER-249` is the
current log-stream return-boundary mypy tranche: the touched log-stream module
is focused strict-mypy clean, and the official wrapper moved from 4609 errors
in 319 files to 4608 errors in 318 files while still blocked. live Linear
issue `JER-250` closed through PR #243 as the scenario playback typing tranche:
the touched playback module is focused strict-mypy clean, and the official
wrapper moved from 4608 errors in 318 files to 4604 errors in 317 files while
still blocked. live Linear issue `JER-251` closed through PR #244 as the
second-system smoke typing tranche: the touched smoke module is focused
strict-mypy clean, and the official wrapper moved from 4604 errors in 317 files
to 4595 errors in 316 files while still blocked. live Linear issue `JER-252`
closed through PR #245 as the report/bundle import-boundary typing tranche: the
touched runtime comparison, fault diagnosis, and workbench bundle modules are
focused strict-mypy clean, and the official wrapper moved from 4595 errors in
316 files to 4579 errors in 313 files while still blocked. live Linear issue
`JER-253` closed through PR #246 as the document-intake/spec typing tranche:
the touched document intake and control-system spec modules are focused
strict-mypy clean, and the official wrapper moved from 4579 errors in 313 files
to 4561 errors in 311 files while still blocked. live Linear issue `JER-254`
closed through PR #247 as the knowledge/hardware-evidence import-boundary
tranche: the touched knowledge capture, hardware registry, and hardware
evidence report modules are focused strict-mypy clean, and the official wrapper
moved from 4561 errors in 311 files to 4556 errors in 308 files while still
blocked. live Linear issue `JER-255` closed through PR #248 as the
reverse-diagnosis typing tranche: the touched reverse diagnosis module is
focused strict-mypy clean, and the official wrapper moved from 4556 errors in
308 files to 4551 errors in 307 files while still blocked. live Linear issue
`JER-256` is the current Monte Carlo typing tranche: the touched Monte Carlo
module is focused strict-mypy clean, and the official wrapper moved from 4551
errors in 307 files to 4550 errors in 307 files while still blocked. Do not
claim full mypy clean.
...
- JER-232 is Done after PR #214.
- JER-233 is Done after PR #215.
- JER-234 is Done after PR #216.
- JER-235 is Done after PR #217.
- JER-236 is Done after PR #218.
- JER-237 is Done after PR #219.
- JER-238 is Done after PR #220.
- Live Linear JER-229 is the post-v5 Linear/Symphony bootstrap issue, not the
  repo-local JER-229 v5 launch slice.
- Live Linear JER-230 is the post-v5 v6 live queue seed issue, not the
  repo-local JER-230 empty-canvas graph authoring slice.
- Live Linear JER-231 is the post-v5 review archive library/recent restore
...
- Live Linear JER-234 is the post-v5 e2e readiness audit issue, not the
  repo-local JER-234 sandbox runner trace kernel v2 slice.
- Live Linear JER-235 is the post-v5 captured-template import gate-fix issue,
  not the repo-local JER-235 debug probe timeline v3 slice.
- Live Linear JER-236 is the post-v5 large sandbox graph trace stability proof,
  not the repo-local JER-236 hardware/interface evidence attachment v2 slice.
- Live Linear JER-237 is the post-JER-236 queue refresh issue, not the
  repo-local JER-237 editor command palette slice.
- Live Linear JER-238 is the post-JER-237 restore diff drilldown issue, not the
  repo-local JER-238 review archive restore/regression bundle v3 slice.
- Live Linear JER-239 is the post-JER-238 large-graph stress pack proof issue.
...
**JER-233 scenario test case library slice**:

- The right inspector now turns the sandbox test bench into a saved
  `scenario_test_case_library` with create, save, duplicate, delete, and select
  controls.
- Each saved test case carries a stable id, name, scenario id, tick inputs,
  assertions, expected outputs, notes, `candidate_state: sandbox_candidate`,
  `certification_claim: none`, and `truth_effect: none`.
- Sandbox run reports still use `workbench-sandbox-test-run-report.v1` and now
  reference the selected/active test case id, graph document id/version,
  graph/workspace revision ids, and `scenario_test_case_library_checksum`.
- Draft export/import, browser restore, editable graph document v2 canonical
  model, evidence archive, and foundation review archive carry the library and
  checksum coverage while preserving legacy single test bench imports.

**JER-234 sandbox runner trace kernel slice**:

- Sandbox run reports keep `workbench-sandbox-test-run-report.v1` and now carry
  nested `sandbox_runner_trace_kernel` as
  `workbench-sandbox-runner-trace-kernel.v2`.
- The browser prepares a deterministic graph view from the canonical draft,
  computes stable source-to-target evaluation order, and records per-tick
  `node_values`, `port_values`, `edge_values`, and `assertion_results`.
- Unsupported ops, dangling edges, duplicate edges, self loops, cycles, and
  ...
  errors make the run report `invalid_scenario` while missing inputs use
  deterministic default values.
- Draft export/import, browser restore, evidence archive, and foundation review
  archive carry the trace kernel through the last run report with
  `sandbox_runner_trace_kernel_checksum`.
- Local evidence passed focused static/e2e tests, adjacent scenario test bench
  e2e, full pytest, adversarial tests, JS syntax check, and diff check. The
  bounded GSD suite was rerun and remains blocked only by the external Notion
  control-plane check timing out/returning TLS EOF; mypy remains the known
  JER-171 baseline blocker.

**JER-235 debug probe timeline slice**:

- The candidate debugger now derives `debug_probe_timeline` from the latest
  sandbox runner trace and selected graph owner.
- Watched node/port values are listed across ticks, with selected tick and
  graph-selection sync metadata kept in the packet.
- The first failing assertion links back to the selected owner when known.
- Draft export/import, browser restore, ChangeRequest proof text, evidence
  archive, and foundation review archive carry `debug_probe_timeline` with
  checksum coverage and `truth_effect: none`.

**JER-236 hardware/interface evidence attachment slice**:

- The workbench now derives `hardware_evidence_attachment_v2` from existing
  hardware bindings, subsystem groups, typed ports, and the hardware interface
  designer model.
- Attachment rows cover node, port, edge, and subsystem-group owners while
  keeping candidate state sandbox-only and `truth_effect: none`.
- Validation reports duplicate attachment ids, missing graph owners, broken
  hardware-interface references, and explicit evidence-gap counts without
  mutating controller truth or hardware YAML.
- Draft export/import, ChangeRequest proof text, evidence archive, and
  foundation review archive carry the packet and checksum coverage.

**JER-237 editor command palette slice**:

- The editor toolbar now exposes a local command palette, and Ctrl/Cmd+K opens
  the same sandbox-only command surface from the keyboard.
- Commands cover create, rename, duplicate, group, wire, run, debug, export,
  import, and archive actions while reusing existing `/workbench` functions.
- Each command records a `command_palette.*` workspace action and keeps canvas
  interaction metadata synchronized with the inspector and archive payloads.
