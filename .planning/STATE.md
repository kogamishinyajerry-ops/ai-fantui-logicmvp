...
- `P6-10 显式化 dashboard-only degraded mode` is now in progress locally: the Notion health validator and dashboard snapshot will treat archived `status / 09C / freeze` pages as an explicit dashboard-only degraded mode instead of reporting a false full-health pass.
