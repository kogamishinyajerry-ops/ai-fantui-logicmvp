...
status: Live Linear JER-239 large-graph stress pack implementing
last_updated: "2026-05-05T14:28:00.000+08:00"
...
is the current restore diff drilldown product slice: mutated local review
archives should report section, checksum key/path, expected/actual checksum,
affected evidence path, and `truth_effect: none`. live Linear issue `JER-239`
is the current large-graph stress pack proof slice: pass, fail, invalid graph,
and stale-report cases should come from reusable fixtures before more archive
or debugger proof is added.
