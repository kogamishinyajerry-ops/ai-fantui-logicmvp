...
- Live Linear `JER-247`: `[project] [L9] [none] [DAL-TBD] JER-171 mypy
  baseline reduction tranche 5` closed through PR #240. This is the fifth
  JER-171 mypy baseline reduction tranche.
- Live Linear `JER-248`: `[project] [L9] [none] [DAL-TBD] JER-171 mypy
  baseline reduction tranche 6` closed through PR #241. This is the sixth
  JER-171 mypy baseline reduction tranche.
- Live Linear `JER-249`: `[project] [L9] [none] [DAL-TBD] JER-171 mypy
  baseline reduction tranche 7` closed through PR #242. This is the seventh
  JER-171 mypy baseline reduction tranche.
- Live Linear `JER-250`: `[project] [L9] [none] [DAL-TBD] JER-171 mypy
  baseline reduction tranche 8` closed through PR #243. This is the eighth
  JER-171 mypy baseline reduction tranche.
- Live Linear `JER-251`: `[project] [L9] [none] [DAL-TBD] JER-171 mypy
  baseline reduction tranche 9` closed through PR #244. This is the ninth
  JER-171 mypy baseline reduction tranche.
- Live Linear `JER-252`: `[project] [L9] [none] [DAL-TBD] JER-171 mypy
  baseline reduction tranche 10` closed through PR #245. This is the tenth
  JER-171 mypy baseline reduction tranche.
- Live Linear `JER-253`: `[project] [L9] [none] [DAL-TBD] JER-171 mypy
  baseline reduction tranche 11` closed through PR #246. This is the eleventh
  JER-171 mypy baseline reduction tranche.
- Live Linear `JER-254`: `[project] [L9] [none] [DAL-TBD] JER-171 mypy
  baseline reduction tranche 12` closed through PR #247. This is the twelfth
  JER-171 mypy baseline reduction tranche.
- Live Linear `JER-255`: `[project] [L9] [none] [DAL-TBD] JER-171 mypy
...
- `[project] [L9] [none] [DAL-TBD] JER-171 mypy baseline reduction tranche 6`
  is live Linear `JER-248`. It types low-risk source return boundaries in
  fault taxonomy and workbench audit logging, keeps the touched source files
  focused strict-mypy clean, and current branch evidence reduces the official
  wrapper from 4611 errors in 321 files to 4609 errors in 319 files while
  still blocked.
- `[project] [L9] [none] [DAL-TBD] JER-171 mypy baseline reduction tranche 7`
  is live Linear `JER-249`. It types the workbench live log stream JSON return
  boundary, keeps the touched module focused strict-mypy clean, and current
  branch evidence reduces the official wrapper from 4609 errors in 319 files
  to 4608 errors in 318 files while still blocked.
- `[project] [L9] [none] [DAL-TBD] JER-171 mypy baseline reduction tranche 8`
  is live Linear `JER-250`. It types scenario playback import and return-value
  boundaries, keeps the touched module focused strict-mypy clean, and current
  branch evidence reduces the official wrapper from 4608 errors in 318 files
  to 4604 errors in 317 files while still blocked.
- `[project] [L9] [none] [DAL-TBD] JER-171 mypy baseline reduction tranche 9`
  is live Linear `JER-251`. It types second-system smoke import and
  selected-id boundaries, keeps the touched module focused strict-mypy clean,
  and current branch evidence reduces the official wrapper from 4604 errors in
  317 files to 4595 errors in 316 files while still blocked.
- `[project] [L9] [none] [DAL-TBD] JER-171 mypy baseline reduction tranche 10`
  is live Linear `JER-252`. It shields report/bundle import boundaries in
  runtime comparison, fault diagnosis, and workbench bundle modules, keeps the
  touched source files focused strict-mypy clean, and current branch evidence
  reduces the official wrapper from 4595 errors in 316 files to 4579 errors in
  313 files while still blocked.
- `[project] [L9] [none] [DAL-TBD] JER-171 mypy baseline reduction tranche 11`
  is live Linear `JER-253`. It types document intake and control-system spec
  import/return boundaries, keeps the touched source files focused
  strict-mypy clean, and current branch evidence reduces the official wrapper
  from 4579 errors in 313 files to 4561 errors in 311 files while still
  blocked.
- `[project] [L9] [none] [DAL-TBD] JER-171 mypy baseline reduction tranche 12`
  is live Linear `JER-254`. It shields knowledge-capture and hardware-evidence
  import boundaries, keeps the touched source files focused strict-mypy clean,
  and current branch evidence reduces the official wrapper from 4561 errors in
  311 files to 4556 errors in 308 files while still blocked.
- `[project] [L9] [none] [DAL-TBD] JER-171 mypy baseline reduction tranche 13`
  is live Linear `JER-255`. It types low-risk reverse-diagnosis import,
  report, and boolean return boundaries, keeps the touched source file focused
  strict-mypy clean, and current branch evidence reduces the official wrapper
  from 4556 errors in 308 files to 4551 errors in 307 files while still
  blocked.
- `[project] [L9] [none] [DAL-TBD] JER-171 mypy baseline reduction tranche 14`
  is live Linear `JER-256`. It types low-risk Monte Carlo import and
  serialization return boundaries, keeps the touched source file focused
  strict-mypy clean, and current branch evidence reduces the official wrapper
...
- JER-234 replaces the old repeated-pass sandbox runner loop with a
  deterministic trace kernel that records node, port, edge, and assertion
...
| P4（回归保护） | system_switcher_smoke URL修复 | ✅ 已完成：DEMO_URL 从 `/` 改为 `/demo.html`，匹配 Phase 3 路由真相。**Phase A (2026-04-22) 更新：chat.html 已搁置，`/` 现直接服务 /demo.html。** (c97d95d) |
...
Status: Done (2026-04-15) · **Shelved in Phase A (2026-04-22) — LLM 依赖已搬到 archive/shelved/llm-features/。以下描述仅作历史参考。**
...
> **Shelved in Phase A (2026-04-22)** — 本 Phase 依赖 LLM (chat/MiniMax/Ollama)，已迁移到 `archive/shelved/llm-features/`。以下描述仅作历史参考。
...
- P21-03 — `archive/shelved/llm-features/scripts/local_model_smoke.py` + 首次真跑（Qwen2.5-7B 3/3 PASS / 4.2–5.4s，报告在 `runs/local_model_smoke_20260418T072226Z/`）
...
**Sub-phases:**
- P22-01 — `archive/shelved/llm-features/scripts/demo_rehearsal_dual_backend.py` + 双后端真跑（MiniMax + Ollama 7B）14/14 PASS，报告在 `runs/demo_rehearsal_dual_backend_20260418T074215Z/`
...
**Sub-phases:**
- P25-01 — `archive/shelved/llm-features/scripts/integrated_timing_rehearsal.py`（段落→API map + 双后端 spawn + degraded 响应检测 + 报告生成）
