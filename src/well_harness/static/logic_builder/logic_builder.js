(function () {
  ...
  function syncOutputBacktraceActiveTrace(traceId) {
    ...
    if (outputBacktraceCoverage) {
      ...
      const coverageTail = relatedLabels.length
        ...
        : "";
      const sourceTail = activeSourceCompactLabel ? `·${activeSourceCompactLabel}` : "";
  ...
  function renderCircuitEvaluationHud(snapshot, nodeById) {
    ...
    setReadableStatusText(circuitReadouts.hudSw1, sw1 && sw1.state === "active" ? "闭合" : "断开");
    setReadableStatusText(circuitReadouts.hudSw2, sw2 && sw2.state === "active" ? "闭合" : "断开");
    setReadableStatusText(circuitReadouts.hudTls, tls && tls.state === "active" ? "已解锁" : "锁定");
    setReadableStatusText(circuitReadouts.hudVdt90, vdt90 && vdt90.state === "active" ? "≥90%" : "待到位");
    if (circuitReadouts.hudLogic) {
      const logicHudText = ["logic1", "logic2", "logic3", "logic4"].map((id, index) => {
        const node = nodeById.get(id);
        return `L${index + 1}:${node && node.state === "active" ? "通" : "-"}`;
      }).join(" · ");
      setReadableStatusText(circuitReadouts.hudLogic, logicHudText);
    }
    if (circuitReadouts.hudThrLock) {
      if (thr && thr.state === "active") setReadableStatusText(circuitReadouts.hudThrLock, "已释放");
      else if (thr && thr.state === "blocked") setReadableStatusText(circuitReadouts.hudThrLock, "已阻断");
      else setReadableStatusText(circuitReadouts.hudThrLock, "-");
    }
