#!/usr/bin/env python3
"""Run the approved Safety output-command conflict repair slice."""
...
from well_harness.agent_development_slice import (
    ...
    DEFAULT_SAFETY_OUTPUT_COMMAND_CONFLICT_ARTIFACT_DIR,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the approved Safety Guardian output-command conflict repair slice.",
    )
    parser.add_argument(
        "--artifact-dir",
        type=Path,
        default=DEFAULT_SAFETY_OUTPUT_COMMAND_CONFLICT_ARTIFACT_DIR,
...
def main() -> int:
    args = _parse_args()
    payload = run_safety_output_command_conflict_candidate_repair_slice(
