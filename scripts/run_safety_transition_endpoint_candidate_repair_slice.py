#!/usr/bin/env python3
"""Run the approved Safety transition endpoint repair slice."""
...
from well_harness.agent_development_slice import (
    ...
    DEFAULT_SAFETY_TRANSITION_ENDPOINT_ARTIFACT_DIR,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the approved Safety Guardian transition endpoint repair slice.",
    )
    parser.add_argument(
        "--artifact-dir",
        type=Path,
        default=DEFAULT_SAFETY_TRANSITION_ENDPOINT_ARTIFACT_DIR,
...
def main() -> int:
    args = _parse_args()
    payload = run_safety_transition_endpoint_candidate_repair_slice(
