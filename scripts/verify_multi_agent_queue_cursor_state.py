...
from scripts.verify_multi_agent_queue_run_ledger import verify_multi_agent_queue_run_ledger


PROJECT_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_CURSOR_ARTIFACT_DIR = Path("/tmp/ai-fantui-multi-agent-queue-cursor-state")
CURSOR_NAME = "multi_agent_queue_cursor_state_v0_1.json"
LEDGER_SCHEMA_NAME = "multi_agent_queue_run_ledger_v0_1.schema.json"
CURSOR_SCHEMA_ID = (
    "https://well-harness.local/json_schema/multi_agent_queue_cursor_state_v0_1.schema.json"
)
CURSOR_SCHEMA_NAME = "multi_agent_queue_cursor_state_v0_1.schema.json"
CURSOR_KIND = "ai-fantui-multi-agent-queue-cursor-state"
CURSOR_ID = "multi-agent-queue-cursor-state-v0.1"
CURSOR_GATE_ID = "multi-agent-queue-cursor-state"
SOURCE_LEDGER_ID = "multi-agent-queue-run-ledger-v0.1"
...
EXPECTED_READY_COMPLETED_RECORD_IDS = EXPECTED_IDLE_COMPLETED_RECORD_IDS[:9]
OPEN_RECORD_ID = "RUN-QUEUE-010"
OPEN_QUEUE_ITEM_ID = "queue-safety-unreachable-state-repair"
OPEN_TASK_ID = "TASK-CE-CHECK-UNREACHABLE-STATE-001"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Directory containing multi_agent_queue_cursor_state_v0_1.json.",
    )
    parser.add_argument(
        "--cursor",
        type=Path,
...
def _check_ready_source_ledger(
    ...
    valid = True
    if len(records) != 10:
        mismatches.append("ready source ledger must contain nine completed records plus one approved open record")
        valid = False
    completed = records[:9]
    open_records = records[9:]
    if [record.get("record_id") for record in completed if isinstance(record, dict)] != (
        EXPECTED_READY_COMPLETED_RECORD_IDS
    ):
        mismatches.append("ready source ledger completed prefix must preserve RUN-QUEUE-001 through RUN-QUEUE-009")
        valid = False
...
def _check_resume_selection(cursor: dict[str, Any], mismatches: list[str]) -> bool:
    ...
    if not isinstance(policy, dict):
        ...
        if policy.get("policy_id") != "first-approved-open-item-v0.1":
            mismatches.append("resume_policy.policy_id must be first-approved-open-item-v0.1")
            valid = False
