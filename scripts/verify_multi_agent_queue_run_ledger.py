...
from scripts.verify_approved_candidate_task_queue_v0_8_artifact import (
    verify_approved_candidate_task_queue_v0_8_artifact,
)
from well_harness.agent_review_packet import validate_candidate_review_packet_export


PROJECT_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_LEDGER_ARTIFACT_DIR = Path("/tmp/ai-fantui-multi-agent-queue-run-ledger")
LEDGER_NAME = "multi_agent_queue_run_ledger_v0_1.json"
LEDGER_SCHEMA_ID = (
    "https://well-harness.local/json_schema/multi_agent_queue_run_ledger_v0_1.schema.json"
)
LEDGER_SCHEMA_NAME = "multi_agent_queue_run_ledger_v0_1.schema.json"
LEDGER_KIND = "ai-fantui-multi-agent-queue-run-ledger"
LEDGER_ID = "multi-agent-queue-run-ledger-v0.1"
LEDGER_GATE_ID = "multi-agent-queue-run-ledger"
SOURCE_QUEUE_ID = "approved-candidate-task-queue-v0.8"
SOURCE_QUEUE_GATE_ID = "approved-candidate-task-queue-v0-8"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Directory containing multi_agent_queue_run_ledger_v0_1.json.",
    )
    parser.add_argument(
        "--ledger",
        type=Path,
...
def _check_source_queue(ledger: dict[str, Any], ledger_path: Path, mismatches: list[str]) -> bool:
    ...
    if source_path is None:
        ...
        return False
    result = verify_approved_candidate_task_queue_v0_8_artifact(summary_path=source_path)
    if result.get("status") != "pass":
        mismatches.append("source queue v0.8 checker must pass")
