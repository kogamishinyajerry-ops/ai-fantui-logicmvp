#!/usr/bin/env python3
"""Run the append-only approved candidate task queue v0.2."""
...
from well_harness.agent_development_slice import (
    ...
    run_requirement_candidate_repair_slice,
)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run approved candidate repair task queue v0.2 after per-task readiness preflight.",
    )
    parser.add_argument(
        "--artifact-dir",
        type=Path,
    ...
    return [
        {
            ...
            "queue_contract": _queue_contract(
                ...
                approval_id="APPROVAL-FIRST-CANDIDATE-REPAIR-SLICE-001",
            ),
        },
        {
        ...
        {
            "queue_item_id": M7_QUEUE_ITEM_ID,
            "slice_id": "requirement-approved-candidate-repair-slice",
            "runner": run_requirement_candidate_repair_slice,
            "queue_contract": _queue_contract(
    ...
    *,
    ...
    if preflight["status"] != "pass":
        return {
            ...
            "queue_contract": dict(item["queue_contract"]),
            "preflight": preflight,
            "execution": {
                "status": "not_run",
...
def run_approved_candidate_task_queue_v0_2(
    *,
    ...
    payload = {
        ...
        "append_only": {
            ...
            "previous_task_count": len(V0_1_QUEUE_ORDER),
            "preserved_prefix_count": len(V0_1_QUEUE_ORDER),
            "new_task_count": len(queue_order) - len(V0_1_QUEUE_ORDER),
            "template_id": TEMPLATE_ID,
        },
        "items": items,
        "deterministic_gates": {
            "m6_template_contract": "pass",
            "append_only_prefix": "pass" if queue_order[:3] == V0_1_QUEUE_ORDER else "fail",
            ...
            "requirement_repair_slice": (
                "pass" if items[-1]["status"] == "pass" else "fail"
            ),
            "local_gate": status,
        },
        "aggregate": {
...
def main() -> int:
    args = _parse_args()
    payload = run_approved_candidate_task_queue_v0_2(
