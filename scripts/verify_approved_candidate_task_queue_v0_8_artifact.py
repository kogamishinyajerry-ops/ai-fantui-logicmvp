#!/usr/bin/env python3
"""Validate approved candidate task queue v0.8 and child review exports."""
...
QUEUE_SUMMARY_SCHEMA_NAME = "approved_candidate_task_queue_summary_v0_8.schema.json"
...
EXPECTED_QUEUE_ORDER = [*V0_7_QUEUE_ORDER, M20_QUEUE_ITEM_ID]
EXPECTED_TASKS = {
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify approved candidate task queue v0.8 summary and child review exports.",
    )
...
def _schema_mismatches(summary: dict[str, Any]) -> list[str]:
    ...
    for error in errors:
        ...
        mismatches.append(
            f"approved candidate task queue v0.8 schema validation failed{location}: {error.message}"
        )
    return mismatches
...
def verify_approved_candidate_task_queue_v0_8_artifact(
    *,
    ...
    if queue_order != EXPECTED_QUEUE_ORDER:
        summary_mismatches.append("queue_order must list the approved v0.8 queue in expected order")
    if not isinstance(queue_order, list) or queue_order[:9] != V0_7_QUEUE_ORDER:
        summary_mismatches.append("v0.8 queue must preserve v0.7 queue order prefix")
        append_only_valid = False
    ...
    if append_only != expected_append_only:
        summary_mismatches.append("append_only must describe one appended v0.8 item from v0.7")
        append_only_valid = False
    ...
    for item in items:
        ...
        if queue_item_id == M20_QUEUE_ITEM_ID:
        ...
        if queue_item_id == M20_QUEUE_ITEM_ID:
            ...
            if not isinstance(finding, dict) or finding.get("code") != "CHECK_UNREACHABLE_STATE_001":
                child_mismatches.append(
                    "queue-safety-unreachable-state-repair review export must include CHECK_UNREACHABLE_STATE_001"
                )
...
def main() -> int:
    args = _parse_args()
    payload = verify_approved_candidate_task_queue_v0_8_artifact(
