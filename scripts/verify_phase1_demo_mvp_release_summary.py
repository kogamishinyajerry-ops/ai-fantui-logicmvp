#!/usr/bin/env python3
"""Verify the Phase 1 demo MVP release summary artifact."""
...
DEFAULT_SUMMARY_PATH = (
    Path("/tmp/ai-fantui-phase1-demo-mvp-release-checklist")
...
def _release_status_valid(summary: dict[str, Any], mismatches: list[str]) -> bool:
    valid = True
    if summary.get("status") != "pass":
        mismatches.append("status must be pass")
        valid = False
    if summary.get("release_status") != "pass":
        mismatches.append("release_status must be pass")
        valid = False
...
def _deterministic_gates_valid(summary: dict[str, Any], mismatches: list[str]) -> bool:
    ...
    valid = True
    for name in ("browser_acceptance", "review_package", "review_package_verification", "ci_artifact"):
    ...
    if checks.get("documentation", {}).get("status") != "pass":
        mismatches.append("checks.documentation.status must be pass")
        valid = False
    return valid
...
def verify_release_summary(summary_path: Path = DEFAULT_SUMMARY_PATH) -> dict[str, Any]:
    mismatches: list[str] = []
    summary = _load_json(summary_path)
    gate_results = {
        "schema": "pass" if _schema_valid(summary, mismatches) else "fail",
        "release_status": "pass" if _release_status_valid(summary, mismatches) else "fail",
    ...
    return {
        "kind": "ai-fantui-phase1-demo-mvp-release-summary-verification",
        "status": status,
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify phase1_demo_mvp_release_summary.json.",
    )
...
def _emit(payload: dict[str, Any], output_format: str) -> None:
    ...
    if payload["status"] == "pass":
        print("PASS: phase1 demo MVP release summary verified")
    else:
        print(f"FAIL: phase1 demo MVP release summary drifted ({', '.join(payload['mismatches'])})")


def main(argv: list[str] | None = None) -> int:
    args = _parse_args(argv or [])
    try:
        payload = verify_release_summary(args.summary)
