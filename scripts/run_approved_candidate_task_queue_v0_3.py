#!/usr/bin/env python3
"""Run the append-only approved candidate task queue v0.3."""
...
from well_harness.agent_development_slice import (
    ...
    run_safety_undefined_signal_candidate_repair_slice,
)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run approved candidate repair task queue v0.3 after M8 fast preflight.",
    )
    parser.add_argument(
        "--artifact-dir",
        type=Path,
...
def _run_prefix_v0_2(
    *,
    ...
    try:
        ...
        raise RuntimeError(f"v0.2 prefix queue did not emit JSON: {result.stderr}") from exc
    if result.returncode != 0 or payload.get("status") != "pass":
        raise RuntimeError(f"v0.2 prefix queue did not pass: {payload.get('status')}")
    return payload
...
def _approved_shell_status(slice_payload: dict[str, Any]) -> dict[str, Any]:
    try:
        loop_path_value = slice_payload.get("artifact_paths", {}).get("repair_loop_result")
...
def _run_m9_queue_item(
    *,
    ...
    if preflight["status"] != "pass":
        return {
            "queue_item_id": M9_QUEUE_ITEM_ID,
    ...
    try:
        slice_payload = run_safety_undefined_signal_candidate_repair_slice(
...
def run_approved_candidate_task_queue_v0_3(
    *,
    artifact_dir: Path = DEFAULT_QUEUE_ARTIFACT_DIR,
    input_packet_path: Path = DEFAULT_AGENT_OUTPUT_FIXTURE_PATH,
) -> dict[str, Any]:
    prefix_payload = _run_prefix_v0_2(
        artifact_dir=artifact_dir / "prefix-v0-2",
        input_packet_path=input_packet_path,
    )
    prefix_items = list(prefix_payload.get("items", []))
    m9_item = _run_m9_queue_item(
        artifact_dir=artifact_dir / M9_QUEUE_ITEM_ID,
        input_packet_path=input_packet_path,
    )
    items = [*prefix_items, m9_item]
    ...
    fast_gate_used = (
        m9_item.get("queue_contract", {}).get("readiness_command") == FAST_GATE_COMMAND
        and m9_item.get("preflight", {}).get("command") == FAST_GATE_COMMAND
        and m9_item.get("preflight", {}).get("status") == "pass"
    )
    payload = {
        ...
        "append_only": {
            ...
            "previous_task_count": len(V0_2_QUEUE_ORDER),
            "preserved_prefix_count": len(V0_2_QUEUE_ORDER),
            "new_task_count": len(queue_order) - len(V0_2_QUEUE_ORDER),
            "template_id": TEMPLATE_ID,
        },
        "items": items,
        "deterministic_gates": {
            "m8_fast_construction_gate": "pass" if fast_gate_used else "fail",
            "append_only_prefix": "pass" if queue_order[:4] == V0_2_QUEUE_ORDER else "fail",
            ...
            "safety_undefined_signal_repair_slice": (
                "pass" if m9_item["status"] == "pass" else "fail"
            ),
            "local_gate": status,
        },
        "aggregate": {
        ...
        "artifact_paths": {
            ...
            "prefix_v0_2_summary": str(prefix_summary_path),
        },
    }
...
def main() -> int:
    args = _parse_args()
    payload = run_approved_candidate_task_queue_v0_3(
