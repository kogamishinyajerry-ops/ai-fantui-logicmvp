#!/usr/bin/env python3
"""Run the first approved candidate repair slice and emit machine-readable evidence."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the first approved Chief Engineer candidate repair slice.",
    )
    parser.add_argument(
        "--artifact-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Candidate agent_output_contract_v0_1 packet to repair.",
    )
    parser.add_argument(
        "--format",
        choices=("text", "json"),
...
def main() -> int:
    args = _parse_args()
    payload = run_first_candidate_repair_slice(
