#!/usr/bin/env python3
"""Validate approved candidate task queue v0.2 and child review exports."""
...
DEFAULT_QUEUE_ARTIFACT_DIR = Path("/tmp/ai-fantui-approved-candidate-task-queue-v0-2")
QUEUE_SUMMARY_NAME = "approved_candidate_task_queue_summary_v0_2.json"
QUEUE_GATE_ID = "approved-candidate-task-queue-v0-2"
QUEUE_ID = "approved-candidate-task-queue-v0.2"
QUEUE_SUMMARY_KIND = "ai-fantui-approved-candidate-task-queue-summary"
QUEUE_SUMMARY_SCHEMA_ID = (
    "https://well-harness.local/json_schema/approved_candidate_task_queue_summary_v0_2.schema.json"
)
QUEUE_SUMMARY_SCHEMA_NAME = "approved_candidate_task_queue_summary_v0_2.schema.json"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify approved candidate task queue v0.2 summary and child review exports.",
    )
    parser.add_argument(
        "--artifact-dir",
        type=Path,
        default=DEFAULT_QUEUE_ARTIFACT_DIR,
        help="Directory containing approved_candidate_task_queue_summary_v0_2.json.",
    )
    parser.add_argument(
        "--summary",
        type=Path,
...
def _schema_mismatches(summary: dict[str, Any]) -> list[str]:
    ...
    for error in errors:
        ...
        mismatches.append(
            f"approved candidate task queue v0.2 schema validation failed{location}: {error.message}"
        )
    return mismatches
...
def _requirement_item_valid(item: dict[str, Any], mismatches: list[str]) -> bool:
    valid = True
    if item.get("queue_item_id") != "queue-requirement-ambiguity-repair":
        mismatches.append("M7 queue item must be queue-requirement-ambiguity-repair")
        valid = False
    contract = item.get("queue_contract")
    if not isinstance(contract, dict):
        mismatches.append("M7 queue item must include queue_contract")
        return False
    expected = EXPECTED_TASKS["queue-requirement-ambiguity-repair"]
    checks = {
        "template_id": TEMPLATE_ID,
    ...
    for key, expected_value in checks.items():
        if contract.get(key) != expected_value:
            mismatches.append(f"M7 queue_contract.{key} must be {expected_value}")
            valid = False
    return valid


def verify_approved_candidate_task_queue_v0_2_artifact(
    *,
    ...
    try:
        ...
        return {
            ...
            "new_requirement_item_valid": False,
    ...
    if queue_order != EXPECTED_QUEUE_ORDER:
        summary_mismatches.append("queue_order must list the approved v0.2 queue in expected order")
    if not isinstance(queue_order, list) or queue_order[:3] != V0_1_QUEUE_ORDER:
        summary_mismatches.append("v0.2 queue must preserve v0.1 queue order prefix")
        append_only_valid = False
    if summary.get("append_only") != _expected_append_only():
        summary_mismatches.append("append_only must describe one-item v0.2 growth from v0.1")
        append_only_valid = False
    ...
    if len(items) != len(EXPECTED_QUEUE_ORDER):
        summary_mismatches.append("items must contain exactly four queue summaries")
    ...
    for index, item in enumerate(items):
        ...
        if queue_item_id == "queue-requirement-ambiguity-repair":
            new_requirement_item_valid = _requirement_item_valid(item, summary_mismatches)
        ...
        if not isinstance(preflight, dict):
            ...
            if preflight.get("command") != "make multi-agent-construction-readiness":
                summary_mismatches.append(
                    f"{queue_item_id}.preflight.command must be make multi-agent-construction-readiness"
                )
        ...
        if queue_item_id == "queue-requirement-ambiguity-repair":
            ...
            if not isinstance(finding, dict) or finding.get("code") != "REQ_AMBIGUITY_UNRESOLVED":
                child_mismatches.append(
                    "queue-requirement-ambiguity-repair review export must include REQ_AMBIGUITY_UNRESOLVED"
                )
    ...
    return {
        ...
        "new_requirement_item_valid": new_requirement_item_valid,
...
def main() -> int:
    args = _parse_args()
    payload = verify_approved_candidate_task_queue_v0_2_artifact(
    ...
    if args.format == "json":
        ...
        print(f"new_requirement_item_valid: {payload['new_requirement_item_valid']}")
