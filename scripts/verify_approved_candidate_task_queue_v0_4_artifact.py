#!/usr/bin/env python3
"""Validate approved candidate task queue v0.4 and child review exports."""
...
QUEUE_SUMMARY_SCHEMA_NAME = "approved_candidate_task_queue_summary_v0_4.schema.json"
...
EXPECTED_QUEUE_ORDER = [*V0_3_QUEUE_ORDER, M12_QUEUE_ITEM_ID]
EXPECTED_TASKS = {
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify approved candidate task queue v0.4 summary and child review exports.",
    )
...
def _schema_mismatches(summary: dict[str, Any]) -> list[str]:
    ...
    for error in errors:
        ...
        mismatches.append(
            f"approved candidate task queue v0.4 schema validation failed{location}: {error.message}"
        )
    return mismatches
...
def verify_approved_candidate_task_queue_v0_4_artifact(
    *,
    ...
    if queue_order != EXPECTED_QUEUE_ORDER:
        summary_mismatches.append("queue_order must list the approved v0.4 queue in expected order")
    if not isinstance(queue_order, list) or queue_order[:5] != V0_3_QUEUE_ORDER:
        summary_mismatches.append("v0.4 queue must preserve v0.3 queue order prefix")
        append_only_valid = False
    ...
    if append_only != expected_append_only:
        summary_mismatches.append("append_only must describe one resumed v0.4 item from v0.3")
        append_only_valid = False
    ...
    for item in items:
        ...
        if queue_item_id == M12_QUEUE_ITEM_ID:
        ...
        if queue_item_id == M12_QUEUE_ITEM_ID:
            ...
            if not isinstance(finding, dict) or finding.get("code") != "EV_BOUNDARY_RESULT_REVIEW_REQUIRED":
                child_mismatches.append(
                    "queue-evidence-boundary-review-repair review export must include EV_BOUNDARY_RESULT_REVIEW_REQUIRED"
                )
...
def main() -> int:
    args = _parse_args()
    payload = verify_approved_candidate_task_queue_v0_4_artifact(
