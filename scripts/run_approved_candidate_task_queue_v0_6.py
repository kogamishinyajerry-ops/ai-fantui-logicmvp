#!/usr/bin/env python3
"""Run the append-only approved candidate task queue v0.6."""
...
from well_harness.agent_development_slice import (
    ...
    run_evidence_unknown_trace_candidate_repair_slice,
)


PROJECT_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_QUEUE_ARTIFACT_DIR = Path("/tmp/ai-fantui-approved-candidate-task-queue-v0-6")
QUEUE_ID = "approved-candidate-task-queue-v0.6"
PREVIOUS_QUEUE_ID = "approved-candidate-task-queue-v0.5"
QUEUE_GATE_ID = "approved-candidate-task-queue-v0-6"
QUEUE_SUMMARY_NAME = "approved_candidate_task_queue_summary_v0_6.json"
QUEUE_SUMMARY_KIND = "ai-fantui-approved-candidate-task-queue-summary"
QUEUE_SUMMARY_SCHEMA_ID = (
    "https://well-harness.local/json_schema/approved_candidate_task_queue_summary_v0_6.schema.json"
)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Execute the M18 append-only approved queue item as v0.6.",
    )
...
def _run_prefix_v0_5(*, artifact_dir: Path, input_packet_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"v0.5 prefix queue did not emit JSON: {result.stderr}") from exc
    if result.returncode != 0 or payload.get("status") != "pass":
        raise RuntimeError(f"v0.5 prefix queue did not pass: {payload.get('status')}")
    return payload
...
def _run_m18_queue_item(*, artifact_dir: Path, input_packet_path: Path) -> dict[str, Any]:
    preflight = _run_fast_gate_preflight(artifact_dir / "fast-gate")
    if preflight["status"] != "pass":
        return {
            "queue_item_id": M18_QUEUE_ITEM_ID,
            "slice_id": "evidence-unknown-trace-approved-candidate-repair-slice",
    ...
    try:
        slice_payload = run_evidence_unknown_trace_candidate_repair_slice(
...
def run_approved_candidate_task_queue_v0_6(
    *,
    artifact_dir: Path = DEFAULT_QUEUE_ARTIFACT_DIR,
    input_packet_path: Path = DEFAULT_AGENT_OUTPUT_FIXTURE_PATH,
) -> dict[str, Any]:
    prefix_payload = _run_prefix_v0_5(
        artifact_dir=artifact_dir / "prefix-v0-5",
        input_packet_path=input_packet_path,
    )
    prefix_items = list(prefix_payload.get("items", []))
    m18_item = _run_m18_queue_item(
        artifact_dir=artifact_dir / M18_QUEUE_ITEM_ID,
        input_packet_path=input_packet_path,
    )
    items = [*prefix_items, m18_item]
    ...
    fast_gate_used = (
        m18_item.get("queue_contract", {}).get("readiness_command") == FAST_GATE_COMMAND
        and m18_item.get("preflight", {}).get("command") == FAST_GATE_COMMAND
        and m18_item.get("preflight", {}).get("status") == "pass"
    )
    payload = {
        ...
        "append_only": {
            ...
            "previous_task_count": len(V0_5_QUEUE_ORDER),
            "preserved_prefix_count": len(V0_5_QUEUE_ORDER),
            "new_task_count": len(queue_order) - len(V0_5_QUEUE_ORDER),
            "template_id": TEMPLATE_ID,
        ...
        "deterministic_gates": {
            ...
            "append_only_prefix": "pass" if queue_order[:7] == V0_5_QUEUE_ORDER else "fail",
            ...
            "evidence_unknown_trace_repair_slice": (
                "pass" if m18_item["status"] == "pass" else "fail"
            ),
            "local_gate": status,
        },
        "aggregate": {
        ...
        "artifact_paths": {
            ...
            "prefix_v0_5_summary": str(prefix_summary_path),
        },
    }
...
def main() -> int:
    args = _parse_args()
    payload = run_approved_candidate_task_queue_v0_6(
