#!/usr/bin/env python3
"""Run the approved Evidence unknown IR trace repair slice."""
...
from well_harness.agent_development_slice import (
    ...
    DEFAULT_EVIDENCE_UNKNOWN_TRACE_ARTIFACT_DIR,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the approved Evidence Agent unknown IR trace repair slice.",
    )
    parser.add_argument(
        "--artifact-dir",
        type=Path,
        default=DEFAULT_EVIDENCE_UNKNOWN_TRACE_ARTIFACT_DIR,
...
def main() -> int:
    args = _parse_args()
    payload = run_evidence_unknown_trace_candidate_repair_slice(
