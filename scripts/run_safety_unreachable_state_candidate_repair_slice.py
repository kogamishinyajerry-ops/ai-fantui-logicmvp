#!/usr/bin/env python3
"""Run the approved Safety unreachable-state repair slice."""
...
from well_harness.agent_development_slice import (
    ...
    DEFAULT_SAFETY_UNREACHABLE_STATE_ARTIFACT_DIR,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the approved Safety Guardian unreachable-state repair slice.",
    )
    parser.add_argument(
        "--artifact-dir",
        type=Path,
        default=DEFAULT_SAFETY_UNREACHABLE_STATE_ARTIFACT_DIR,
...
def main() -> int:
    args = _parse_args()
    payload = run_safety_unreachable_state_candidate_repair_slice(
