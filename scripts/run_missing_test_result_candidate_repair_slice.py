#!/usr/bin/env python3
"""Run the approved missing-test-result candidate repair slice."""
...
from well_harness.agent_development_slice import (
    ...
    DEFAULT_MISSING_TEST_RESULT_ARTIFACT_DIR,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the approved missing-test-result Evidence candidate repair slice.",
    )
    parser.add_argument(
        "--artifact-dir",
        type=Path,
        default=DEFAULT_MISSING_TEST_RESULT_ARTIFACT_DIR,
...
def main() -> int:
    args = _parse_args()
    payload = run_missing_test_result_candidate_repair_slice(
