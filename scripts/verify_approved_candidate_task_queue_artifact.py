#!/usr/bin/env python3
"""Validate the approved candidate task queue artifact and child exports."""
...
QUEUE_SUMMARY_NAME = "approved_candidate_task_queue_summary.json"
...
QUEUE_SUMMARY_SCHEMA_NAME = "approved_candidate_task_queue_summary_v0_1.schema.json"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify approved candidate task queue summary and child review exports.",
    )
    parser.add_argument(
        "--artifact-dir",
        type=Path,
        default=DEFAULT_QUEUE_ARTIFACT_DIR,
        help="Directory containing approved_candidate_task_queue_summary.json.",
    )
    parser.add_argument(
        "--summary",
        type=Path,
...
def _schema_mismatches(summary: dict[str, Any]) -> list[str]:
    ...
    for error in errors:
        ...
        mismatches.append(
            f"approved candidate task queue summary schema validation failed{location}: {error.message}"
        )
    return mismatches
...
def verify_approved_candidate_task_queue_artifact(
    *,
    ...
    if summary.get("queue_order") != EXPECTED_QUEUE_ORDER:
        summary_mismatches.append("queue_order must list the approved queue in expected order")
    ...
    if len(items) != len(EXPECTED_QUEUE_ORDER):
        summary_mismatches.append("items must contain exactly three queue summaries")
...
def main() -> int:
    args = _parse_args()
    payload = verify_approved_candidate_task_queue_artifact(
