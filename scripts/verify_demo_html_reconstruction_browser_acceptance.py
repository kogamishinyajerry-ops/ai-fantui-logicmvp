...
def verify_browser_acceptance(artifact_dir: Path) -> dict[str, Any]:
        ...
        with sync_playwright() as pw:
            ...
            try:
                ...
                first_screen_review = {
                    ...
                    "circuit_snapshot_preview_visible": page.locator(
                        "#demo-reconstruction-circuit-snapshot-preview"
                    ).is_visible(timeout=5000),
                    "circuit_snapshot_preview_count": page.locator(
                        "[data-circuit-snapshot-preview-step]"
                    ).count(),
                ...
                compact_runway_initial_review = page.evaluate(
                    """() => {
                        ...
                        const outputs = () => ({
                            tls: text("#demo-reconstruction-compact-runway-output-tls"),
                            etrac: text("#demo-reconstruction-compact-runway-output-etrac"),
                ...
                source_map_review = page.evaluate(
                    """() => {
                        ...
                        return {
                            ...
                            circuitSnapshotPreviewCount: document.querySelectorAll("[data-circuit-snapshot-preview-step]").length,
                            circuitSnapshotPreviewReadback: text("#demo-reconstruction-circuit-snapshot-preview-readback"),
                ...
                page.locator('[data-circuit-snapshot-preview-step="runway-l4-thr-lock"]').click()
                page.wait_for_function(
                    """() => {
                        const selected = document.querySelector("#demo-reconstruction-selected-anchor")?.textContent || "";
                        const active = document.querySelector('[data-circuit-snapshot-preview-step="runway-l4-thr-lock"][aria-pressed="true"]');
                ...
                review_index_review = page.evaluate(
                    """() => {
                        ...
                        return {
                            ...
                            proofPathText: text("#demo-reconstruction-review-index-proof-path"),
                            handoffCount: document.querySelectorAll("[data-review-index-handoff]").length,
                            handoffSummaryText: text("#demo-reconstruction-review-index-handoff-summary"),
                            handoffSourceText: text('[data-review-index-handoff="source"]'),
                            handoffOutputText: text('[data-review-index-handoff="output"]'),
                            ...
                            activeTour: Array.from(
                                document.querySelectorAll("[data-review-index-tour][aria-pressed='true']")
                            ).map((button) => button.getAttribute("data-review-index-tour")),
                            evidenceCount: document.querySelectorAll("[data-review-index-evidence]").length,
                            evidenceSummaryText: text("#demo-reconstruction-review-index-evidence-summary"),
                            evidenceSourceText: text('[data-review-index-evidence="docx-source"]'),
                            evidenceCoverageText: text('[data-review-index-evidence="object-coverage"]'),
                            activeEvidence: Array.from(
                                document.querySelectorAll("[data-review-index-evidence][aria-pressed='true']")
                            ).map((button) => button.getAttribute("data-review-index-evidence")),
                ...
                page.locator("#demo-reconstruction-review-index-complete-action").click()
                page.wait_for_function(
                    """() => {
                        const selected = document.querySelector("#demo-reconstruction-selected-anchor");
                ...
                review_index_complete_action = page.evaluate(
                    """() => ({
                        selectedAnchor: document.querySelector("#demo-reconstruction-selected-anchor")?.textContent?.trim() || "",
                        activeScenarios: Array.from(
                            document.querySelectorAll("[data-review-index-scenario][aria-pressed='true']")
                ...
                page.wait_for_function(
                    """() => {
                        const selected = document.querySelector("#demo-reconstruction-selected-anchor");
                        const active = document.querySelector('[data-review-index-handoff="output"][aria-pressed="true"]');
                        const proofTarget = document.querySelector('[data-review-index-target="demo-reconstruction-proof-path"][aria-pressed="true"]');
                        const objectText = document.querySelector("#demo-reconstruction-review-index-object")?.textContent || "";
                        return selected
                            && selected.textContent.trim() === "P035-S05"
                ...
                review_index_handoff_rail_action = page.evaluate(
                    """() => ({
                        selectedAnchor: document.querySelector("#demo-reconstruction-selected-anchor")?.textContent?.trim() || "",
                        activeHandoff: Array.from(
                            document.querySelectorAll("[data-review-index-handoff][aria-pressed='true']")
                ...
                page.locator('[data-review-index-tour="closure"]').click()
                page.wait_for_function(
                    """() => {
                        const active = document.querySelector('[data-review-index-tour="closure"][aria-pressed="true"]');
                        const target = document.querySelector("#demo-reconstruction-review-index-closure-rail");
                        return !!active && !!target && window.scrollY > 0;
                    }""",
                    timeout=5000,
                )
                review_index_tour_rail_action = page.evaluate(
                    """() => ({
                        activeTour: Array.from(
                            document.querySelectorAll("[data-review-index-tour][aria-pressed='true']")
                        ).map((button) => button.getAttribute("data-review-index-tour")),
                ...
                page.locator('[data-review-index-evidence="object-coverage"]').click()
                page.wait_for_function(
                    """() => {
                        const active = document.querySelector('[data-review-index-evidence="object-coverage"][aria-pressed="true"]');
                        const targetButton = document.querySelector('[data-review-index-target="demo-reconstruction-coverage-matrix"][aria-pressed="true"]');
                        const target = document.querySelector("#demo-reconstruction-coverage-matrix");
                        return !!active && !!targetButton && !!target && window.scrollY > 0;
                    }""",
                    timeout=5000,
                )
                review_index_evidence_rail_action = page.evaluate(
                    """() => ({
                        activeEvidence: Array.from(
                            document.querySelectorAll("[data-review-index-evidence][aria-pressed='true']")
                        ).map((button) => button.getAttribute("data-review-index-evidence")),
                        ...
                        evidenceSummaryText: document.querySelector("#demo-reconstruction-review-index-evidence-summary")?.textContent?.trim() || "",
                        scrollY: window.scrollY,
                    })"""
                )
                page.locator('[data-review-index-output-target="thr_lock"]').click()
                page.wait_for_function(
                    """() => {
                        const selected = document.querySelector("#demo-reconstruction-selected-anchor");
                        const active = document.querySelector('[data-review-index-output-target="thr_lock"][aria-pressed="true"]');
                        const proofTarget = document.querySelector('[data-review-index-target="demo-reconstruction-proof-path"][aria-pressed="true"]');
                ...
                review_index_output_rail_action = page.evaluate(
                    """() => ({
                        selectedAnchor: document.querySelector("#demo-reconstruction-selected-anchor")?.textContent?.trim() || "",
                        activeOutputTargets: Array.from(
                            document.querySelectorAll("[data-review-index-output-target][aria-pressed='true']")
    ...
    gates = {
        ...
        "circuit_snapshot_preview": "pass"
        if (
            circuit_snapshot_preview_action["activePreview"] == ["runway-l4-thr-lock"]
            and circuit_snapshot_preview_action["activeChain"] == ["runway-l4-thr-lock"]
            and circuit_snapshot_preview_action["selectedAnchor"] == "P035-S05"
        ...
        if (
            ...
            and "20/20 节点" in review_index_review["evidenceCoverageText"]
        ...
        if (
            review_index_complete_action["selectedAnchor"] == "P035-S05"
        ...
        if (
            review_index_handoff_rail_action["selectedAnchor"] == "P035-S05"
            and review_index_handoff_rail_action["activeHandoff"] == ["output"]
            and review_index_handoff_rail_action["activeTargets"] == ["demo-reconstruction-proof-path"]
            and "wire_logic4_thr_lock" in review_index_handoff_rail_action["objectText"]
            and "对象" in review_index_handoff_rail_action["proofPathText"]
            and "wire_logic4_thr_lock" in review_index_handoff_rail_action["hash"]
            and review_index_handoff_rail_action["scrollY"] > 0
        )
        else "fail",
        ...
        if (
            review_index_output_rail_action["selectedAnchor"] == "P035-S05"
            and review_index_output_rail_action["activeOutputTargets"] == ["thr_lock"]
            and review_index_output_rail_action["activeTargets"] == ["demo-reconstruction-proof-path"]
            and "wire_logic4_thr_lock" in review_index_output_rail_action["objectText"]
            and "对象" in review_index_output_rail_action["proofPathText"]
            and "链接已同步" in review_index_output_rail_action["proofPathText"]
            and "lane=object" in review_index_output_rail_action["hash"]
            and "wire_logic4_thr_lock" in review_index_output_rail_action["hash"]
            and review_index_output_rail_action["scrollY"] > 0
        )
        else "fail",
        ...
        if (
            review_index_tour_rail_action["activeTour"] == ["closure"]
            and review_index_tour_rail_action["scrollY"] > 0
        )
        else "fail",
        "review_index_evidence_rail": "pass"
        if (
            review_index_evidence_rail_action["activeEvidence"] == ["object-coverage"]
            and review_index_evidence_rail_action["activeTargets"] == ["demo-reconstruction-coverage-matrix"]
            and "20/20 节点" in review_index_evidence_rail_action["evidenceSummaryText"]
            and review_index_evidence_rail_action["scrollY"] > 0
        )
        else "fail",
