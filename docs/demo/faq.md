...
- R-锚点：**R2 · R5**
- 证据：`archive/shelved/llm-features/src/llm_client.py` · `src/well_harness/static/adversarial_test.py`
...
**A:** 不绑定。`archive/shelved/llm-features/src/llm_client.py` 是 Protocol + 工厂模式，
...
- R-锚点：**R4**
- 证据：`archive/shelved/llm-features/src/llm_client.py` · `archive/shelved/llm-features/tests/test_llm_client.py`（19 tests 覆盖适配器契约）
...
- 证据：`requirements.txt`（无 anthropic 依赖）· `archive/shelved/llm-features/src/llm_client.py`（adapter 只含 MiniMax + Ollama）
...
前缀命名以复用前端降级 UI。`archive/shelved/llm-features/tests/test_llm_client.py` 里的 MiniMax/Ollama 测试
就是模板。

- 证据：`archive/shelved/llm-features/src/llm_client.py`（当前 ~220 行，含 2 个 backend）
