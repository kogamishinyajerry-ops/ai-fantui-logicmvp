...
- **Adapter 边界：** `archive/shelved/llm-features/src/llm_client.py`（~220 行，Protocol + 2 实现 + factory）。
...
| R5 对抗守门 | Ollama 输出经相同 JSON schema + VALID_ACTION_TYPES / VALID_RESPONSE_TYPES 校验，不可信 payload 直接降级为默认响应。`archive/shelved/llm-features/tests/test_llm_client.py` 19 tests + 主 pytest 658 pass + e2e 49/49 + adversarial 8/8。 |
...
- 灾难演练：`archive/shelved/llm-features/scripts/local_model_smoke.py --skip-if-unreachable` 是幂等、CI-safe 入口。
