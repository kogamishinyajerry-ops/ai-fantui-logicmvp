...
[Point] If the audience asks what the node colors mean, point to the visible `状态图例 / truth boundary`: `Active` means the backend snapshot currently lights the node, `Blocked` means the node is waiting on named conditions, and `Inactive` means the current route has not entered that node yet. The same strip also separates controller truth from simplified plant feedback.

[Point] When you switch to the lever cockpit's `当前结论`, read it top-to-bottom as `Headline -> Blocker -> Next step`. That reading rail is still the same lever snapshot payload, not a second presenter-only explanation layer.

[Point] If someone asks whether the result area changed payloads, point to the visible source note in `结果摘要`: it now tells you whether the page is showing `POST /api/demo / DemoAnswer` or `POST /api/lever-snapshot`, and it reminds the audience that the visible result panes still share one payload.

[Point] When the page is in `DemoAnswer` mode, the lever `当前结论` rail should switch into a visible hold state instead of reusing the last lever snapshot. That keeps the left-side rails honest with the source note instead of silently carrying stale lever evidence.
...
8. Confirm the visible `状态图例 / truth boundary` explains `Active / Blocked / Inactive` and distinguishes controller truth from simplified plant feedback.
9. Confirm the visible `当前结论` rail reads in the fixed order `Headline -> Blocker -> Next step` and does not introduce a second answer payload.
10. Confirm the visible source note in `结果摘要` distinguishes `POST /api/demo / DemoAnswer` from `POST /api/lever-snapshot` while still saying the panes share one payload.
11. Confirm that when `DemoAnswer` is active, the lever `当前结论` rail shows the visible hold state instead of reusing a stale lever snapshot.
12. Confirm the control chain highlights `logic4 / THR_LOCK`, the highlight explanation names the answer association, the structured answer is readable, and the `Answer sections` summary shows counts.
13. Expand or review the raw JSON debug panel when you need the machine-readable `DemoAnswer` payload.
14. If the UI shows loading, empty prompt, API error, or network error, treat that as a UI state, not a control-logic conclusion.
15. Restate the boundary before presenting: deterministic controlled demo layer, built-in `nominal-deploy` / `retract-reset` scenarios, simplified first-cut plant, not a full LLM, and not a complete physical model.
