...
|-----------|-------------|--------|-----------------|-----------|--------------|--------------|-------|
| `thrust-reverser` | `certified` | `In use` | `uploads/20260409-thrust-reverser-control-logic.docx`（原 docx, 2026-04-09 · SHA256 `6e457fe3…276133a5`）+ `docs/thrust_reverser/requirements_supplement.md`（P37 反向补完, 2026-04-20）| **Kogami 内部自签**（原 docx 为内部来源历史 snapshot · supplement 为反向补完 · 非外部权威 · 见 supplement §7）| — | 已 certified · `docs/thrust_reverser/traceability_matrix.md` · **truth lives in controller.py + yaml** · **workbench spec via `system_spec.current_reference_workbench_spec()` (system_spec.py:273, 约 280 行, P41 discovered) · consumed by `controller_adapter.py::ReferenceDeployControllerAdapter.load_spec()`** · **`thrust_reverser_intake_packet.py` 保留 Lean (business fields = ()) per D1=A intake-layer choice** · **Appendix A 6 项全部 resolved via P37 supplement**（§2-§7）· 若需外部 certification-grade authority 需新 Phase 升级 · supplement §7.3 列升级路径 | P36β (2026-04-20) 完成 intake packet 120 行精益版 + YAML 头 + matrix 241 行 · P37 (2026-04-20) 完成 supplement 297 行 + intake 加第 4 SourceDocumentRef + matrix Appendix A 6 项 ⚠️→✅ + YAML 头 supplement block · P41 (2026-04-20) 澄清 D1=A intake-layer 语义 + supplement §1.4/§8 discovery 注 · controller.py / models.py / YAML values / current_reference_workbench_spec 字节级不变 |
